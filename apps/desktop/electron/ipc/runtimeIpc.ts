...
import { URL } from 'url'
import { getRuntimeToken, TOKEN_HEADER } from '../security/runtime-token'
...
function httpRequest(
  ...
  return new Promise((resolve, reject) => {
    ...
    // P0-4: attach the runtime token for Go Core requests.
    const token = getRuntimeToken()
    if (token && parsedUrl.origin === new URL(CORE_URL).origin) {
      ;(options.headers as Record<string, string>)[TOKEN_HEADER] = token
    }

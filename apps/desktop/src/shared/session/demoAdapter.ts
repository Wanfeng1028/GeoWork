/**
 * demoAdapter — 演示模式专用 mock 流式（D1，doc/21 §0）
 *
 * 【仅演示模式可达】本文件是原 streamAdapters.ts 中 340 行 mock 的整体迁入地。
 * 生产默认路径不会执行到这里：只有 isDemoModeEnabled() 显式开启（?demo=1 或
 * localStorage geowork.demo.enabled）且 core 不可达时，Session.send 才调用
 * mockStreamAdapter，并通过快照 isDemo=true 驱动 UI 常驻"演示模式"标注。
 *
 * 事件以 StreamAdapterCallbacks 回调形态输出（结构化事件），不接触 React。
 */

import type { RunStatus, SelectedContextItem, StreamAdapter, WorkflowStep, WorkMode } from './types'
    ...
    return new Promise<void>((resolve) => {
      const tick = () => {
        ...
        if (index >= fullText.length) {
          ...
          callbacks.onStatus?.('waiting-confirmation' as RunStatus)
          callbacks.onDone()
          clearAllTimers()
          resolve()
          return

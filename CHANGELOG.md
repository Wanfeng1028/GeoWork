...
### Docs — doc/26 §3.2 侧栏结论翻案补记：D-27-10 ① 提前落档（2026-08-20 · ZCode）
- **doc/26**：§3.2「侧栏 Conversations 保留自研」结论标注翻案（doc/27 D-27-10 ①）——原判理由（扁平列表 + menu 模型不匹配）被 antdx Conversations API 逐项核对推翻，新结论 = W2-4 迁移 antdx Conversations；原理由存档不删；表下加范围注（其余三行结论仍有效）；头部状态行与关联文档行补翻案指向
- **doc/27**：W2-4 验收项 ② 加注"已提前完成"；§9 执行记录补翻案补记行
- 影响代码文件：无（零代码改动）

### Docs — 文档排版整顿：AGENT.md 结构修复 + checklist 过期项改实（2026-08-20 · ZCode）
- **AGENT.md**：章节物理顺序修正（§14 施工记录移回 §15 文档治理之前，恢复 13→14→15→16）；版本表 v1.2/v1.6 重复行合并（编号不重排，v1.4/v1.13 等外部引用不断）；§1 当前阶段巨型单元格瘦身（完整时间线归 §14 与 CHANGELOG）；§3 文档路由表补 v0.6 施工入口（doc/27 + ADR-002/003），apps/desktop 行标注"当前施工以 doc/27 排期为准"；§15.1 层级树与 §15.2 Owner 表补 doc/27；§14 补第 0 周施工记录；版本表上方补"版本历史"标题
- **DEV_VERSION_CHECKLIST**：过期项改实——E2E Playwright ✅ 已就位（doc/20 锚点 + doc/24 P7-1 testbed + CI nightly）；依赖安全扫描 ✅ 已集成（P6 门禁：npm audit / govulncheck / pip-audit）；MSW 行改口径（方案未采用，集成覆盖由契约测试 + E2E spec 承担）；单测条目改"覆盖率阈值待定；114 条全绿"；F0-1 复选框 [x]→[ ] 矛盾修正
- **doc/27**：W1 第 0 项 CI 触发文案收口（七 job 名单落字面）；第 1 周标题补唯一例外括号（/api/usage 诱饵端点提前砍）；§9 执行记录补档行提交哈希落实 + 追加本轮行
- **ADR-002**：接线前提补第 6 条 2026-08-20 补记（同 PR 砍 /api/diffs 六条活路由，§10.3-4 拍板）；文末补 Related: ADR-003
- 影响代码文件：无（零代码改动）

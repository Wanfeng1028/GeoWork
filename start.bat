...
cd /d "%ROOT_DIR%core"
start "GeoWork-Go-Core" cmd /c "go run ./cmd/geowork-runtime --port !GO_PORT! > "%ROOT_DIR%logs\go-core.log" 2>&1"
cd /d "%ROOT_DIR%"
...
cd /d "%ROOT_DIR%workers\geo-python"
start "GeoWork-Python-Worker" cmd /c "python -m uvicorn app.main:app --host 127.0.0.1 --port !PY_PORT! > "%ROOT_DIR%logs\python-worker.log" 2>&1"
cd /d "%ROOT_DIR%"

:: 等待启动
timeout /t 3 >nul

:: 获取 Python Worker 进程 PID
...
cd /d "%ROOT_DIR%"
start "GeoWork-Desktop" cmd /c "set ELECTRON_EXEC_PATH=!ELECTRON_PATH! && npm --workspace apps/desktop run dev > "%ROOT_DIR%logs\desktop.log" 2>&1"
